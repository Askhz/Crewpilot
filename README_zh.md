...
| **security-reviewer** | OWASP Top 10 安全审计——注入、认证、XSS、密钥泄露 | CRITICAL 发现阻止发布 |
| **code-simplifier** | 代码简化——去死代码、消抽象、砍重复 | 每删一行，少一个未来的 bug |
| **debugger** | 四阶段根因分析——复现、隔离、诊断、处方 | 没有证据不修复 |
| **designer** | UI/UX 设计审查——一致性、交互、响应式、无障碍 | 五个维度 0-10 评分 |
